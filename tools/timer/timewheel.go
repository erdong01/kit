...
func NewTimeWheel(name string, interval int64, scales int, maxCap int) *TimeWheel {
	...
	log.Println("Init timerWhell name = ", tw.name, " is Done!")
	return tw
}
...
func (tw *TimeWheel) AddTimeWheel(next *TimeWheel) {
	...
	log.Println("Add timerWhell[", tw.name, "]'s next [", next.name, "] is succ!")
}
...
func (tw *TimeWheel) Run() {
	go tw.run()
	log.Println("timerwheel name = ", tw.name, " is running...")
}
