...
func GenerateNo(l int, head ...string) string {
	...
	if uuidStr == "" {
		...
		for range remainingLen {
			// 约束：随机部分的第一位不能为 0
			if sb.Len() == headLen {
				sb.WriteByte(strDigitsNonZero[rand.IntN(len(strDigitsNonZero))])
				continue
			}
	...
	uuidCharIndex := len(uuidChars) // 当前从uuidChars中取字符的索引

	// 填充ID直到达到长度l
	for sb.Len() < l {
		if uuidCharIndex <= 0 {
			// 如果UUID的字符已经用完，但仍需更多数字，则重新生成。
			uuidStr = New()
			if uuidStr == "" {
				// 重新生成也失败：兜底用随机数填充剩余部分，避免越界panic。
				remainingLen := l - sb.Len()
		...
		// 约束：随机部分的第一位不能为 0
		if sb.Len() == headLen {
			digit = strDigitsNonZero[charByte%9] // charByte % 9 的结果范围是0-8
		} else {
